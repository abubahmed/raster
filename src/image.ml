...
let compare ~image1 ~image2 ~count =
  let acc, store =
    foldi ~init:(0, []) image1 ~f:(fun ~x ~y (acc, store) pixel ->
      let my_pixel = get image2 ~x ~y in
      if not (Pixel.equal my_pixel pixel)
      then acc + 1, store @ [ pixel, my_pixel ]
      else acc, store)
  in
  if acc > 0 then print_s [%message "# of mismatches" (acc : int)];
  let store_sub =
    List.sub store ~pos:0 ~len:(if acc > count then count else acc)
  in
  List.iter store_sub ~f:(fun (first, second) ->
    print_s [%message (first : Pixel.t) (second : Pixel.t)]);
  ()
;;


let compare_fast ~image1 ~image2 =
  let acc =
    foldi ~init:0 image1 ~f:(fun ~x ~y acc pixel ->
      let my_pixel = get image2 ~x ~y in
      if not (Pixel.equal my_pixel pixel)
      then acc + 1
      else acc)
